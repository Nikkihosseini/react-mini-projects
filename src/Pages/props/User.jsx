            ...
            <div className="flex items-center justify-center w-screen min-h-screen bg-slate-800 overflow-x-hidden">
                <div className="bg-stone-200 w-[292px] h-auto p-5 rounded-md text-stone-900 text-center shadow-md shadow-stone-100">
                   <div className="flex items-center justify-between">
                     <button className="text-sm bg-inherit flex items-center gap-1 outline-none border-none focus:outline-none" onClick={() => deleteUser(id)}>
                ...
                <h1 className="text-xl font-bold mb-1">{name}</h1>
                    ...
                    {showMore && <span className="block mb-1">{email}</span>}
                    {showMore && <span className="block">{description}</span>}
                    <div>
