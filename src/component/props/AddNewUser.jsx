        ...
        <>
           <div className="flex flex-col items-center justify-center w-screen min-h-screen mx-auto">
                ...
                <div className="flex flex-col items-center justify-center w-[292px] h-[379px] bg-foggy-lilac p-5 mt-5  shadow-[4px_4px_0px_rgba(255,255,0,0.5)]">
                    ...
                    <input onChange={(e) => setNewUser(prev =>({...prev , name: e.target.value}))} className="bg-stone-400 text-purple-glitch w-full mb-3 px-1" value={newUser.name} type="text" name="fullName" id="fullName"/>

                    <label className="text-cosmic-indigo w-full font-bold mb-1.5" htmlFor="fullName">Phone:</label>
                    <input onChange={(e) => setNewUser(prev =>({...prev , phone: e.target.value}))} className="bg-stone-400 text-purple-glitch w-full mb-3 px-1" type="text" name="phone" id="phone" value={newUser.phone}/>

                    <label className="text-cosmic-indigo w-full font-bold mb-1.5" htmlFor="fullName">Username:</label>
                    <input onChange={(e) => setNewUser(prev =>({...prev , username: e.target.value}))} className="bg-stone-400 text-purple-glitch w-full mb-3 px-1" type="text" name="username" id="username" value={newUser.username}/>

                    <label className="text-cosmic-indigo w-full font-bold mb-1.5" htmlFor="fullName">Email:</label>
                    <input onChange={(e) => setNewUser(prev =>({...prev , email: e.target.value}))} className="bg-stone-400 text-purple-glitch w-full mb-3 px-1" type="email" name="email" id="email" value={newUser.email}/>

                    <label className="text-cosmic-indigo font-bold w-full mb-1.5" htmlFor="fullName">Website:</label>
                    <input onChange={(e) => setNewUser(prev =>({...prev , website: e.target.value}))} className="bg-stone-400 text-purple-glitch w-full mb-3 px-1" type="text" name="website" id="website" value={newUser.website}/>
                </div>
